...
//Create a function that can check if the Integer is contained in an array of integer, return boolean
        ...
        scrumTeam1.forEach(  (k, v) -> System.out.println(k+" : "+v) );//for one statement we dont need to use curly braces

        System.out.println("---------------------------------------------");

        //1. create a function that takes two integers and returns the maximum integer

        BiFunction<Integer,Integer,Integer> maxNum = (a,b)->(a>b)? a : b;//ternary
        ...
        BiFunction<int[],int[], List<Integer>> merge = (x,y)->{//x = int[] arr, y = int[] arr, we're gonna merge them in the List
            
            List<Integer> result = new ArrayList<>();
/*
            for (int each : x) { In lambda expression we dont use unnecessary curly braces
                result.add(each);
            }
 ...
 */
        ...
        BiFunction<List<String>, List<Integer>, Map<String, Integer>>  merge2 = (j, k) ->{
            Map<String, Integer> map = new HashMap<>();//return type is Map
            for (int i = 0; i < j.size(); i++) {
                map.put( j.get(i),  k.get(i)); //j.get(i): get names, k.get(i): get scores
            }
            return map;
        };
