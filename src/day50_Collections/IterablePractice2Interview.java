...
public class IterablePractice2Interview {
    ...
    public static void main(String[] args) {
        ...
        Iterator<String> it = names.iterator();//We are calling iterator() method
        //Retrun type is iterator also

        while(it.hasNext()){
            if(it.next().equalsIgnoreCase("ahmed")){//if we skip that line it'll remove every element
                it.remove();
            }
        }
        ...
        while(q.hasNext()){//hasNext each time checks the next element
